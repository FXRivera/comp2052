...
class LoginForm(FlaskForm):
    username = StringField("Username",  # definir input box username
                           validators=[DataRequired(), length(min=3)],
                           render_kw={"placeholder": "Your email"})
    password = PasswordField("Password",  # definir input box password
                           validators=[DataRequired()],
                           render_kw={"placeholder": "Your password"})
    submit = SubmitField("Login",  # definir el botón de submit
                           render_kw={"class": "btn btn-primary"})  # aplicar clases de CSS
    email = EmailField("Email",  # definir input box email
                         validators=[DataRequired()],
